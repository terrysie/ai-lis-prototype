...
当前版本仅实现样本签收数据读取，不实现真实确认签收、退样、补采等写入操作。页面中的签收、标记异常、生成退样、生成补采任务等按钮仍为原型演示交互，不会修改数据库。

后续会逐步加入样本状态更新、退样记录、补采任务和审计日志，让签收操作形成可追溯的完整数据库闭环。
...
Electron 桌面版质控看板页面现在会优先通过安全的 preload / IPC 桥接，从本地 SQLite 数据库的 `instruments`、`qc_events`、`reagent_batches`、`reagent_expiry_alerts`、`test_items` 表读取质控、仪器、试剂批次和近效期预警数据，并用读取结果更新质控看板总览指标、仪器状态列表、质控事件队列、试剂批次列表和试剂近效期预警列表。

GitHub Pages 网页版和普通浏览器环境没有 Electron API，也没有本地 SQLite 能力，因此质控看板页面会继续使用 `index.html` 中已有的静态 fallback 模拟数据；如果 Electron IPC 调用失败，页面同样会保留静态模拟数据并输出 `console.warn`，避免影响静态网页部署。

当前版本仅实现质控看板读取，不实现质控事件处理、试剂更换、暂停项目、生成采购提醒等写入操作。页面中的质控处理和试剂相关按钮仍为原型演示交互，不会修改数据库。

后续会逐步加入质控事件处理、试剂状态更新、采购提醒和审计日志，让质控、试剂、仪器状态和审核拦截形成完整可追溯流程。
